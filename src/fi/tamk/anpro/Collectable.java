	...
	public void setActive()
	{
		...
		while (!isPlaced) {
	        ...
	        for (int i = wrapper.obstacles.size()-1; i >= 0; --i) {
		        if (Math.abs(x - wrapper.obstacles.get(i).x) > (Wrapper.gridSize + 300) && Math.abs(x - wrapper.player.x) > 250 &&
		        	Math.abs(y - wrapper.obstacles.get(i).y) > (Wrapper.gridSize + 300) && Math.abs(y - wrapper.player.y) > 500) {
	        		isPlaced = true;
	        		break;
				}
	        }
